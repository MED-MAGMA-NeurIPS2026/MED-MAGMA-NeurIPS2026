...
def interior(Q, tol=1e-6, max_iter=200, step=1e-3):
    def objective(s):
        return s.T @ Q @ s
    ...
    def constraint(s):
        return np.sum(np.log(s))  # should equal 0

    def constraint_jac(s):
        return 1 / s

    def constraint_hess(s, v):
        # v is the Lagrange multiplier
        return np.diag(-v / (s**2))

    # Initial guess (positive and product near 1)
    s0 = np.ones(Q.shape[0])
    ...
    bounds = [(1e-8, None) for _ in range(Q.shape[0])]  # s_i > 0
    ...
    # print("Optimal s:", res.x)
    # print("Product:", np.prod(res.x))
    # print("Objective:", objective(res.x))

    return res.x

def interior_exp(Q, old_s, tol=1e-6, max_iter=200, step=1e-3):
    def objective(ell):
        s = np.exp(ell)
        return s @ Q @ s
    ...
    def hess(ell):
        s = np.exp(ell)
        Qs = Q @ s
        return 2 * (np.diag(s) @ Q @ np.diag(s) + np.diag(s * Qs))

    # linear constraint: sum ell = 0
    A = np.ones((1, Q.shape[0]))
    #lc = LinearConstraint(A, -np.log(old_s).sum(), -np.log(old_s).sum())
    lc = LinearConstraint(A, 0, 0)

    ell0 = np.zeros(Q.shape[0])
...
def flip(Omega1, Omega2, Y, s2, tol=1e-6, max_iter=200, step=1e-3):

    d2 = Omega2.shape[0]

    Omega_tilde = get_Omega_tilde(Omega1, Omega2, Y, s2.reshape(1, d2))

    return interior_exp(Omega_tilde, old_s=s2, tol=tol, max_iter=max_iter, step=step)
    

def flop(Omega1, Omega2, Y, s1, tol=1e-6, max_iter=200, step=1e-3):
    d1 = Omega1.shape[0]

    Omega_tilde = get_Omega_tilde(Omega2, Omega1, Y.T, s1.reshape(1, d1))

    return interior_exp(Omega_tilde, old_s=s1, tol=tol, max_iter=max_iter, step=step)
...
def flipflop(Omega1, Omega2, Y, s1, s2, max_iter=20, convergence_tol=1e-10, zero_tol=1e-4, verbose=False):
    ...
    # Divide by norms to stabilize against numerical issues - it is really surprising how well this works!!
    # Doesn't matter theoretically, b/c all we need is to pick a fiber, and the fiber is invariant to scaling.
    return new_s1 / np.linalg.norm(new_s1), new_s2 / np.linalg.norm(new_s2)
...
def laplace_approximation(Omega1, Omega2, Y, s1, s2):
    ...
    zstar, s1, s2 = get_zstar(Omega1, Omega2, Y, s1, s2, max_iter=200, convergence_tol=1e-3, zero_tol=1e-5)
